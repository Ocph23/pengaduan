<template>
  <div>
    <div class="page-header">
      ...
      <a @click="openAddModal" class="btn-primary">+ Tambah User</a>
    </div>
          ...
          <tr v-for="user in users" :key="user.id">
            ...
            <td>
              <div class="action-group">
                ...
                <button class="action-btn delete" title="Hapus" @click="confirmDelete(user)">
                  🗑️
                </button>
              </div>
            </td>
      ...
      <form @submit.prevent="saveuser">
        <div class="modal-body">
          <div class="form-group">
            <label for="name">Nomor Induk <span class="required">*</span></label>
            <input id="name" v-model="form.nomor_induk" type="text" required autofocus />
          </div>
          <div class="form-group">
          ...
          <div class="form-group" v-if="!isEdit">
            ...
            <input id="name" v-model="form.username" type="text" required autofocus />
          </div>
          <div class="form-group" v-if="!isEdit">
            <label for="name">Password<span class="required">*</span></label>
            <input id="name" v-model="form.password" type="text" required autofocus />
          </div>
          <div class="form-group">
            <label for="name">Nomor Telepon<span class="required">*</span></label>
            <input id="name" v-model="form.phone_number" type="text" required autofocus />
          </div>
          <div class="form-group">
            <label for="role">Password<span class="required">*</span></label>
            <select class="filter-select" v-model="form.role">
              <option>Semua Status</option>
  ...
  <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="cancelDelete">
    <div class="modal modal-sm">
      <div class="modal-body" style="text-align: center; padding: 32px 24px;">
        ...
        <h3 style="margin: 12px 0 8px; color: #1e293b;">Hapus Pengguna?</h3>
        ...
        <div class="modal-footer" style="justify-content: center;">
          ...
          <button class="btn-danger" :disabled="isSubmitting" @click="deleteuser">
...
const fetchUsers = async () => {
  ...
  try {
    ...
    const errorMsg = err.data?.statusMessage || err.message || 'Gagal memuat Pengguna'
...
const openEditModal = (user) => {
  ...
  form.nomor_induk = user.nomor_induk;
  form.role = user.role;
  showModal.value = true
}
...
const saveuser = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      const res = await $fetch(`/api/users/${editingId.value}`, {
        method: 'PUT',
        body: data
      })
      showToast(res?.message || 'Pengguna berhasil diperbarui', 'success')
    } else {
    ...
    const message = error.data?.statusMessage || error.message || 'Gagal menyimpan Pengguna'
...
const deleteuser = async () => {
  ...
  try {
    ...
    const message = error.data?.statusMessage || error.message || 'Gagal menghapus Pengguna'
    showToast(message, 'error')
  } finally {
    isSubmitting.value = false
  }
