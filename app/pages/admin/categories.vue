    ...
    <div class="card">
      <div class="card-toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" placeholder="Cari kategori..." class="search-input" />
        </div>
      </div>
      ...
      <div class="table-wrapper">
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-else v-for="cat in filteredCategories" :key="cat.id">
      ...
      <div class="table-footer">
        <span>Menampilkan {{ filteredCategories.length }} dari {{ categories.length }} kategori</span>
      </div>
    </div>
    ...
    <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="cancelDelete">
      <div class="modal modal-sm">
        <div class="modal-body" style="text-align: center; padding: 32px 24px;">
          ...
          <h3 style="margin: 12px 0 8px; color: #1e293b;">Hapus Kategori?</h3>
...
const fetchCategories = async () => {
  ...
  try {
    ...
    const errorMsg = err.data?.statusMessage || err.message || 'Gagal memuat kategori'
...
const saveCategory = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      showToast(res?.message || 'Kategori berhasil diperbarui', 'success')
    } else {
      const res = await $fetch('/api/categories', {
        method: 'POST',
        body: data
      })
      showToast(res?.message || 'Kategori berhasil ditambahkan', 'success')
    }

    closeModal()
    await fetchCategories()
  } catch (error) {
    const message = error.data?.statusMessage || error.message || 'Gagal menyimpan kategori'
...
const deleteCategory = async () => {
  ...
  isSubmitting.value = true
  try {
    const res = await $fetch(`/api/categories/${categoryToDelete.value.id}`, {
      method: 'DELETE'
    })
    showToast(res?.message || 'Kategori berhasil dihapus', 'success')
    cancelDelete()
    await fetchCategories()
  } catch (error) {
    const message = error.data?.statusMessage || error.message || 'Gagal menghapus kategori'
    showToast(message, 'error')
  } finally {
    isSubmitting.value = false
  }
